...
if (!fs.existsSync(envPath)) {
  ...
  console.error(chalk.yellow(`📍 Esperado em: ${envPath}`));
  process.exit(1);
}
...
function validateEnvironment() {
  ...
  ENV_RULES.optional.forEach(rule => {
    ...
    if (isValid && value) {
      ...
      console.log(chalk.gray(`  ⏭️  ${rule.name} (não configurado, mas opcional)`));
    } else {
      console.log(chalk.yellow(`  ⚠️  ${rule.name}`));
  ...
  if (process.env.NODE_ENV === 'development' || envConfig.NODE_ENV === 'development') {
    const jwtSecret = process.env.JWT_SECRET || envConfig.JWT_SECRET;
    if (jwtSecret === 'dev_jwt_secret_key_change_in_production') {
      console.log(chalk.yellow(`  ⚠️  JWT_SECRET usando valor padrão de desenvolvimento`));
      results.warnings.push('JWT_SECRET está usando valor padrão - está correto para desenvolvimento');
    } else {
    ...
    const jwtSecret = process.env.JWT_SECRET || envConfig.JWT_SECRET;
    if (jwtSecret === 'dev_jwt_secret_key_change_in_production') {
      console.log(chalk.red(`  ❌ JWT_SECRET usando valor PADRÃO EM PRODUÇÃO!`));
      results.critical.errors.push({
        var: 'JWT_SECRET',
        type: 'security',
  ...
  console.log(chalk.yellow(`  🟠 Importantes: ${results.important.passed}/${totalImportant} OK`));
  console.log(chalk.blue(`  🟡 Opcionais:   ${results.optional.passed}/${totalOptional} OK`));
  ...
  if (results.critical.failed === 0) {
    console.log(chalk.green.bold('✅ VALIDAÇÃO PASSOU - Variáveis críticas OK!'));
    console.log(chalk.cyan('━'.repeat(60)));
    return { success: true, hasWarnings: results.warnings.length > 0 };
  } else {
    console.log(chalk.red.bold('❌ VALIDAÇÃO FALHOU - Erros críticos encontrados!'));
...
function printHelpText() {
  ...
  console.log(chalk.cyan(`     docker compose --env-file .env.production up -d\n`));
}
