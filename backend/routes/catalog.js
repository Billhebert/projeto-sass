...
router.get('/categories/:categoryId', authenticateToken, async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${ML_API_BASE}/categories/${categoryId}`
    );
 ...
 */
router.get('/categories/:categoryId/attributes', authenticateToken, async (req, res) => {
  try {
    const { categoryId } = req.params;
...
router.get('/trends/:categoryId', authenticateToken, async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${ML_API_BASE}/trends/${SITE_ID}/${categoryId}`
    ).catch(err => {
...
/**
 * GET /api/catalog/:accountId/eligibility/:itemId
 * Check if item is eligible for catalog
 */
router.get('/:accountId/eligibility/:itemId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
 ...
 */
router.post('/:accountId/publish/:itemId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    const { accountId, itemId } = req.params;
 ...
 */
router.delete('/:accountId/publish/:itemId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    const { accountId, itemId } = req.params;
    const account = req.mlAccount;
 ...
 */
router.get('/:accountId/competition/:itemId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    const { accountId, itemId } = req.params;
    const account = req.mlAccount;
    ...
    res.json({
      ...
      data: {
        ...
        metrics: {
          ...
          my_position: myItem ? results.findIndex(r => r.id === itemId) + 1 : null,
        },
      },
    });
  } catch (error) {
 ...
 */
router.get('/:accountId/buybox/:itemId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    const { accountId, itemId } = req.params;
    const account = req.mlAccount;
 ...
 */
router.get('/:accountId/items', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    ...
    // Get item details and filter for catalog items
    const catalogItems = [];
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        eligibleItems: Math.max(0, totalItems - catalogCount),
        buyboxWins,
        catalogPercentage: totalItems > 0 ? Math.round((catalogCount / totalItems) * 100) : 0,
      },
    });
  } catch (error) {
    logger.error({
