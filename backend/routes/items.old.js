  ...
  async (req, res) => {
    try {
      ...
      if (all === "true") {
        ...
        // Fetch all item IDs
        while (true) {
          const params = { limit: batchSize, offset: currentOffset };
        ...
        // Fetch details for all items in batches of 20 to avoid overwhelming the API
        const allItems = [];
        for (let i = 0; i < allItemIds.length; i += 20) {
          ...
          allItems.push(...itemsData.filter((item) => item !== null));
        }
      ...
      // Fetch details for each item (limit to 20 for performance)
  ...
  async (req, res) => {
    try {
      ...
      const items = await Promise.all(
        itemIds.map((itemId) =>
          axios
            .get(`${ML_API_BASE}/items/${itemId}`, {
              headers,
              params: {
                attributes:
                  "id,title,price,currency_id,available_quantity,status,thumbnail,category_id",
              },
            })
            .then((res) => ({
              id: res.data.id,
        ...
        itemIds.slice(0, 20).map(async (itemId) => {
          try {
            const [itemRes, visitsRes] = await Promise.all([
              ...
              axios
                .get(`${ML_API_BASE}/items/${itemId}/visits/time_window`, {
                  headers,
            ...
            return {
              ...
              roi:
                estimatedCost > 0
                  ? ((profit / estimatedCost) * 100).toFixed(1)
                  : 0,
            };
          } catch (e) {
            return null;
  ...
  async (req, res) => {
    try {
      ...
      const totalRevenue = items.reduce(
        (acc, i) => acc + i.price * i.soldQuantity,
        0,
      );
      const totalProfit = items.reduce(
        (acc, i) => acc + i.profit * i.soldQuantity,
        0,
      );
      ...
      res.json({
        ...
        stats: {
          ...
          profitableItems:
            Math.round((profitableItems / items.length) * totalItems) || 0,
          unprofitableItems:
            Math.round((unprofitableItems / items.length) * totalItems) || 0,
          totalRevenue: totalRevenue,
          totalProfit: totalProfit,
          avgPrice:
            items.length > 0
              ? (
                  items.reduce((acc, i) => acc + i.price, 0) / items.length
                ).toFixed(2)
              : 0,
        },
      });
  ...
  async (req, res) => {
    try {
      ...
      const [itemRes, descRes, visitsRes] = await Promise.all([
        axios.get(`${ML_API_BASE}/items/${itemId}`, { headers }),
        axios
          .get(`${ML_API_BASE}/items/${itemId}/description`, { headers })
          .catch(() => ({ data: null })),
        axios
          .get(
            `${ML_API_BASE}/items/${itemId}/visits/time_window?last=30&unit=day`,
            { headers },
          )
          .catch(() => ({ data: null })),
      ]);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create item on ML
      const response = await axios.post(`${ML_API_BASE}/items`, itemData, {
        headers,
      });
 ...
 */
router.get(
  "/:accountId/:itemId/variations/:variationId",
  authenticateToken,
  validateMLToken("accountId"),
  async (req, res) => {
    try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Validate required fields
      if (
        !variationData.attribute_combinations ||
        !Array.isArray(variationData.attribute_combinations)
      ) {
        return res.status(400).json({
          success: false,
  ...
  async (req, res) => {
    try {
      ...
      if (updateData.available_quantity !== undefined) {
        variations[variationIndex].available_quantity =
      ...
      if (updateData.attribute_combinations !== undefined) {
        variations[variationIndex].attribute_combinations =
          updateData.attribute_combinations;
      }
      ...
      res.json({
        ...
        data: response.data.variations?.find(
          (v) => v.id.toString() === variationId,
        ),
      });
    } catch (error) {
    ...
    try {
      ...
      if (updatedVariations.length === 0 && variations.length > 1) {
        return res.status(400).json({
          success: false,
          message:
            "Cannot delete all variations. Item must have at least one variation or none.",
        });
      }
 ...
 */
router.put(
  "/:accountId/:itemId/variations/:variationId/stock",
  authenticateToken,
  validateMLToken("accountId"),
  async (req, res) => {
    try {
