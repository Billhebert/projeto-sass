...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const { limit = 20, offset = 0, isRead, sort = '-dateCreated' } = req.query;
...
router.get('/:accountId/unread', authenticateToken, async (req, res) => {
  try {
    ...
    const messages = await Message.findUnread(accountId, { limit: parseInt(limit) });
...
router.get('/:accountId', authenticateToken, async (req, res) => {
  try {
    const { accountId } = req.params;
    const { limit = 20, offset = 0, packId, isRead, sort = '-dateCreated' } = req.query;
 ...
 */
router.get('/:accountId/pack/:packId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    const { accountId, packId } = req.params;
    const account = req.mlAccount;

    // Fetch conversation from ML API
    const headers = {
    ...
    const response = await axios.get(
      `${ML_API_BASE}/messages/packs/${packId}/sellers/${account.mlUserId}`,
      { headers }
    ).catch(err => {
      logger.warn({
    ...
    res.json({
      ...
      data: {
        packId,
        messages: response?.data?.messages || localMessages.map(m => m.getDetails()),
        paging: response?.data?.paging || { total: localMessages.length },
      },
    });
  } catch (error) {
    logger.error({
 ...
 */
router.post('/:accountId/pack/:packId', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    const { accountId, packId } = req.params;
    ...
    const message = new Message({
      accountId,
      userId: req.user.userId,
      mlMessageId: response.data.id?.toString() || `local_${Date.now()}`,
      packId,
      text: text.trim(),
      from: {
        id: account.mlUserId,
...
router.post('/:accountId/sync', authenticateToken, validateMLToken('accountId'), async (req, res) => {
  try {
    ...
    // Get recent orders with pack IDs
      ...
      try {
        ...
        if (response.data?.messages) {
          allMessages = allMessages.concat(
            response.data.messages.map(m => ({ ...m, packId }))
          );
        }
      } catch (err) {
        logger.warn({
    ...
    // Save messages
    const savedMessages = await saveMessages(accountId, req.user.userId, account.mlUserId, allMessages);
...
async function saveMessages(accountId, userId, sellerId, mlMessages) {
  ...
  for (const mlMessage of mlMessages) {
    try {
      const isSeller = mlMessage.from?.user_id?.toString() === sellerId.toString();
      
      const messageData = {
        accountId,
        userId,
        mlMessageId: mlMessage.id?.toString() || `ml_${Date.now()}_${Math.random()}`,
        ...
        dateCreated: mlMessage.date_created ? new Date(mlMessage.date_created) : new Date(),
        dateReceived: mlMessage.date_received ? new Date(mlMessage.date_received) : null,
