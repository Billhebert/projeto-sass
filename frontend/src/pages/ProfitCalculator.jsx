...
function ProfitCalculator() {
  ...
  const loadItems = async () => {
    ...
    setError(null)
    try {
      const response = await api.get(`/items/${selectedAccount}/profit-analysis`)
      setItems(response.data.items || [])
    } catch (err) {
      // Se a rota nao existe, carrega items normais
      try {
        const itemsResponse = await api.get(`/items/${selectedAccount}`)
        const itemsList = itemsResponse.data.items || []
        // Adiciona dados de lucro simulados
        setItems(itemsList.map(item => ({
          ...item,
          cost: Number(item.price) * 0.4, // Simulacao: custo = 40% do preco
  ...
  const loadStats = async () => {
    try {
      const response = await api.get(`/items/${selectedAccount}/profit-stats`)
      setStats(response.data.stats || stats)
    } catch (err) {
      // Calcula stats localmente se API nao disponivel
      if (items.length > 0) {
        const profitable = items.filter(i => Number(i.margin || 0) > 0).length
        const avgMargin = items.reduce((acc, i) => acc + Number(i.margin || 0), 0) / items.length
        setStats({
  ...
  const calculateProfit = () => {
    const { salePrice, productCost, shippingCost, mlFeePercent, mlFeeFixed, taxPercent, otherCosts } = calculatorData
    ...
    const markup = productCost > 0 ? ((salePrice - productCost) / productCost) * 100 : 0
  ...
  const sortedItems = [...items].sort((a, b) => {
    switch (sortBy) {
      case 'margin_desc': return (b.margin || 0) - (a.margin || 0)
      case 'margin_asc': return (a.margin || 0) - (b.margin || 0)
  ...
  return (
    <div className="profit-calculator-page">
      ...
      <div className="profit-content">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>Configure os custos dos seus produtos para ver a analise de lucratividade</p>
          </div>
        ) : (
                ...
                {filteredItems.map(item => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td>
                        <span className="fee">{formatCurrency(item.mlFee)}</span>
                      </td>
                      <td>
                        <span className="shipping">{formatCurrency(item.shippingCost)}</span>
                      </td>
                      <td>
                        <span className={`profit ${item.profit < 0 ? 'negative' : 'positive'}`}>
                        ...
                        <div className="margin-cell">
                          <div className="margin-bar">
                            <div 
                              ...
                              style={{ width: `${Math.min(Math.abs(Number(item.margin || 0)), 100)}%` }}
                            ></div>
                          </div>
                          <span className="margin-value">{Number(item.margin || 0).toFixed(1)}%</span>
                        </div>
                      </td>
                      <td>
      ...
      {/* Calculator Modal */}
      {showCalculatorModal && (
        <div className="modal-overlay" onClick={() => setShowCalculatorModal(false)}>
          <div className="modal calculator-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>
                <span className="material-icons">calculate</span>
                Calculadora de Lucro
              </h2>
              <button className="close-btn" onClick={() => setShowCalculatorModal(false)}>
                ...
                <div className="input-section">
                  ...
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData({...calculatorData, salePrice: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  ...
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData({...calculatorData, productCost: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  ...
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData({...calculatorData, shippingCost: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  ...
                  <div className="input-row">
                    <div className="input-group">
                      ...
                      <input
                        ...
                        onChange={(e) => setCalculatorData({...calculatorData, mlFeePercent: parseFloat(e.target.value) || 0})}
                      />
                    </div>
                    <div className="input-group">
                      ...
                      <input
                        ...
                        onChange={(e) => setCalculatorData({...calculatorData, mlFeeFixed: parseFloat(e.target.value) || 0})}
                      />
                    </div>
                  </div>
                  ...
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData({...calculatorData, taxPercent: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  ...
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setCalculatorData({...calculatorData, otherCosts: parseFloat(e.target.value) || 0})}
                    />
                  </div>

                  <button className="btn btn-primary btn-block" onClick={calculateProfit}>
                    <span className="material-icons">calculate</span>
                    Calcular
                  </button>
                </div>
                  ...
                  {calculatorResult ? (
                    <div className="result-content">
                      <div className="result-summary">
                        <div className={`profit-display ${calculatorResult.profit >= 0 ? 'positive' : 'negative'}`}>
                          <span className="label">Lucro por Unidade</span>
                          <span className="value">{formatCurrency(calculatorResult.profit)}</span>
                        </div>
                        <div className={`margin-display ${calculatorResult.margin >= 0 ? 'positive' : 'negative'}`}>
                          <span className="label">Margem de Lucro</span>
                          <span className="value">{calculatorResult.margin.toFixed(2)}%</span>
                        </div>
                      </div>
                      ...
                      <div className="breakdown">
                        ...
                        <div className="breakdown-item">
                          ...
                          <span className="positive">{formatCurrency(calculatorResult.salePrice)}</span>
                        </div>
                        <div className="breakdown-item">
                          <span>(-) Custo do Produto</span>
                          <span className="negative">{formatCurrency(calculatorResult.productCost)}</span>
                        </div>
                        <div className="breakdown-item">
                          <span>(-) Taxa Mercado Livre</span>
                          <span className="negative">{formatCurrency(calculatorResult.mlFee)}</span>
                        </div>
                        <div className="breakdown-item">
                          <span>(-) Frete</span>
                          <span className="negative">{formatCurrency(calculatorResult.shippingCost)}</span>
                        </div>
                        <div className="breakdown-item">
                          <span>(-) Impostos</span>
                          <span className="negative">{formatCurrency(calculatorResult.tax)}</span>
                        </div>
                        <div className="breakdown-item">
                          <span>(-) Outros Custos</span>
                          <span className="negative">{formatCurrency(calculatorResult.otherCosts)}</span>
                        </div>
                        <div className="breakdown-total">
                          <span>= Lucro Liquido</span>
                          <span className={calculatorResult.profit >= 0 ? 'positive' : 'negative'}>
                      ...
                      <div className="metrics">
                        <div className="metric">
                          ...
                          <span className="metric-value">{calculatorResult.markup.toFixed(2)}%</span>
                        </div>
                        <div className="metric">
                          <span className="metric-label">Custo Total</span>
                          <span className="metric-value">{formatCurrency(calculatorResult.totalCosts)}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
