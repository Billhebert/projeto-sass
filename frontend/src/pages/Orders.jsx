...
function Orders() {
      ...
      if (ordersList.length === 0 && !hasTriedSync) {
        ...
        try {
          const syncResponse = await api.post(
            `/orders/${selectedAccount}/sync`,
            {
              status: "paid",
              days: 90,
              all: true, // Fetch ALL orders without limit
            },
          );
          ...
          if (syncErr.response?.data?.code?.startsWith("ML_")) {
            setError(
              `Erro de token ML: ${syncErr.response.data.message}. Por favor, reconecte sua conta.`,
            );
          }
  ...
  const loadOrders = async (setLoadingState = true) => {
    ...
    try {
      ...
      params.append("all", "true"); // Fetch ALL orders without limit
    ...
    <div className="orders-page">
      ...
      {showModal &&
        ...
        (() => {
          ...
          const orderId =
            selectedOrder.id ||
            selectedOrder.mlOrderId ||
            selectedOrder.ml_order_id;
          const dateCreated =
            selectedOrder.dateCreated || selectedOrder.date_created;
          const totalAmount =
            selectedOrder.totalAmount || selectedOrder.total_amount || 0;
          const currencyId =
            selectedOrder.currencyId || selectedOrder.currency_id || "BRL";
          const orderItems =
            selectedOrder.orderItems || selectedOrder.order_items || [];
                  ...
                  <div className="order-detail-section">
                    ...
                    <div className="order-items-list">
                      {orderItems.map((item, idx) => {
                        ...
                        const itemThumbnail =
                          item.thumbnail || item.item?.thumbnail;
